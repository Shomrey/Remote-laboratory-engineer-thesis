...
def create_tables():
    global c
    #Tabela studentow
    c.execute('''CREATE TABLE Students (id integer PRIMARY KEY, name text , surname text , mail text,  password text NOT NULL)''')
    conn.commit()
    #Tabela nauczycieli
    c.execute('''CREATE TABLE Teachers (id integer PRIMARY KEY, name text, surname text, mail text, password text NOT NULL)''')
    conn.commit()
    #Tabela labow
...
def add_student(id:int, name:str, surname:str, mail: str, password: str)-> bool:
    global c
    sql = ''' INSERT INTO Students(id, name, surname, mail, password) VALUES (?,?,?,?,?)'''
    
    try:
        c.execute(sql, (id,name, surname, mail, password))     
        conn.commit()
        return True
    except:
...
def add_teacher(id:int, name:str, surname:str, mail: str, password: str)-> bool:
    global c
    sql = ''' INSERT INTO Teachers(id, name, surname, mail, password) VALUES (?,?,?,?,?)'''
